...
{	
	...
	getline(cin, logare);
	ifstream f;
	f.open(logare, ios::in);
	if(f)
	{
		float* preturi;
		string numeFisierT;
		CaracteristiciEvenimentText a;
		f >> a;
		CaracteristiciEveniment b(a.getZiT(), a.getLunaT(), a.getAnT(), a.getOraT(), a.getMinutT(), a.getDenumireT(), a.getTipT(), a.getMentiuniT());
		CaracteristiciLocatieText c;
		f >> c;
		preturi = new float[c.getNrZone()];
		for (int i = 0; i < c.getNrZone();i++)
		{
			f >> preturi[i];
		}
		Bilet* bileteT;
		bileteT = new Bilet[c.getMaxLocuri()];
		int bilAux = 0;
		bileteT->setAdresa(c.getAdresa());
		for (int i = 0; i < c.getNrZone(); i++)
		{
			for (int j = 0; j < c.getZoind(i).getNrRanduri(); j++)
			{
				for (int q = 0; q < c.getZoind(i).getNrScaunePeRand(j); q++)
				{
					Bilet biletAux(0, b.getDenumire(), b.getTip(), c.locFinal(i, j + 1, q + 1), preturi[i], b.data(b.getZi(), b.getLuna(), b.getAn()), b.oraStr(b.getOra(), b.getMinut()), b.getMentiuni());
					bileteT[bilAux++] = biletAux;
				}
			}
		}
		f >> numeFisierT;
		ofstream f(numeFisierT, ios::out | ios::binary);
		for (int i = 0; i < c.getMaxLocuri(); i++)
		{
			int auxX = bileteT[i].getId();
			int length;

			float auxX2 = bileteT[i].getPret();
			string auxS;

			f.write((char*)&auxX, sizeof(auxX));
			f.write((char*)&auxX2, sizeof(bileteT[i].getPret()));


			auxS = bileteT[i].getAdresa();
			length = auxS.length();
			f.write((char*)&length, sizeof(length));
			f.write(auxS.c_str(), length + 1);

			auxS = bileteT[i].getDenumireEveniment();
			length = auxS.length();
			f.write((char*)&length, sizeof(length));
			f.write(auxS.c_str(), length + 1);

			auxS = bileteT[i].getTipStr();
			length = auxS.length();
			f.write((char*)&length, sizeof(length));
			f.write(auxS.c_str(), length + 1);

			auxS = bileteT[i].getLocBilet();
			length = auxS.length();
			f.write((char*)&length, sizeof(length));
			f.write(auxS.c_str(), length + 1);

			auxS = bileteT[i].getData();
			length = auxS.length();
			f.write((char*)&length, sizeof(length));
			f.write(auxS.c_str(), length + 1);

			auxS = bileteT[i].getOra();
			length = auxS.length();
			f.write((char*)&length, sizeof(length));
			f.write(auxS.c_str(), length + 1);

			auxS = bileteT[i].getMentiuni();
			length = auxS.length();
			f.write((char*)&length, sizeof(length));
			f.write(auxS.c_str(), length + 1);
